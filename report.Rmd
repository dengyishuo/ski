...
plot_stock_analysis <- function(company_data, company_name) {
  # 蜡烛图
  candlestick_plot <- ggplot(company_data, aes(x = date)) +
    tidyquant::geom_candlestick(aes(open = open, high = high, low = low, close = close)) +
    labs(title = paste(company_name, "蜡烛图"), subtitle = "OHLC数据") +
  ...
  sma_price_plot <- ggplot(company_data, aes(x = date)) +
    ...
    geom_point(aes(y = close, color = factor(sma_signal)),
               shape = 21, size = 0.3, alpha = 0.7) +
    geom_point(aes(y = close, color = factor(price_signal)),
               shape = 21, size = 0.3, alpha = 0.7) +
    scale_color_manual(values = c("价格" = "black", "SMA5" = "blue", "SMA10" = "purple", "SMA20" = "orange",
                                  "0" = "gray", "1" = "green", "-1" = "red", "2" = "yellow")) +
    labs(title = paste(company_name, "SMA和价格线图"), subtitle = "SMA指标与价格走势") +
    theme_minimal(base_family = "SimHei") +
    theme(legend.position = "top") +
    scale_x_date(date_labels = "%Y-%m-%d",
                 limits = c(as.Date(Sys.Date()-60), as.Date(Sys.Date()))
  ...
  kdj_price_plot <- ggplot(company_data, aes(x = date)) +
    ...
    geom_point(aes(y = k, color = factor(kdj_signal)),
               shape = 21, size = 0.3, alpha = 0.7) +
    scale_color_manual(values = c("价格" = "black", "K线" = "blue", "D线" = "red", "J线" = "green", "0" = "gray", "1" = "green", "-1" = "red")) +
    labs(title = paste(company_name, "KDJ和价格线图"), subtitle = "KDJ指标与价格走势") +
    theme_minimal(base_family = "SimHei") +
    theme(legend.position = "top") +
    scale_x_date(date_labels = "%Y-%m-%d",
                 limits = c(as.Date(Sys.Date()-60), as.Date(Sys.Date()))
                 )
  kdj_plot <- candlestick_plot/kdj_price_plot

  # MACD和价格线图
  macd_price_plot <- ggplot(company_data, aes(x = date)) +
    geom_line(aes(y = macd, color = "MACD线"), size = 0.5) +
    geom_line(aes(y = signal_line, color = "MACD信号线"), size = 0.5) +
    geom_point(aes(y = macd, color = factor(macd_signal)),
               shape = 21, size = 0.3, alpha = 0.7) +
    scale_color_manual(values = c("价格" = "black", "MACD线" = "blue", "MACD信号线" = "red", "0" = "gray", "1" = "green", "-1" = "red")) +
    labs(title = paste(company_name, "MACD和价格线图"), subtitle = "MACD指标与价格走势") +
    theme_minimal(base_family = "SimHei") +
    theme(legend.position = "top") +
    scale_x_date(date_labels = "%Y-%m-%d",
                 limits = c(as.Date(Sys.Date()-60), as.Date(Sys.Date()))
  ...
  macd_plot <- candlestick_plot/macd_price_plot
  
  return(list(sma_price_plot, kdj_plot, macd_plot))
}
...
aggregate_signal <- map2_df(indicator_results, names(indicator_results), function(data, company_name) {
  data %>%
    ...
    select(date, company, kdj_signal, sma_signal, price_signal, macd_signal)
})

# 定义特定时间段
start_date <- zoo::as.Date("2025-05-01")
end_date <- Sys.Date()
